<template>
  <div style="left: 0; top: 0; width: 100%; height: 100%; position: absolute">
    ...
    <input id="date_" class="selects" value='输入日期' @change="initDate" style="left: 40%; top: 0">
    <input id="city1" class="selects" value='济南市' @change="initCity1" style="right: 40%; top: 0">
    <input id="city2" class="selects" value='青岛市' @change="initCity2" style="left: 47%; top: 0">
  </div>
</template>
...
// eslint-disable-next-line no-unused-vars
let y = 2022, m = 4, d = 5, date = '2022-04-05', city1 = '济南市', city2 = "青岛市"
...
export default {
  ...
  methods: {
    initDate() {
      let ev = document.getElementById('date_')
      date = ev.value
      this.initEditAttribute()
    },
    initCity1() {
      let ev = document.getElementById('city1')
      city1 = ev.value
      this.initEditAttribute()
    },
    initCity2() {
      let ev = document.getElementById('city2')
      city2 = ev.value
      this.initEditAttribute()
    },
    ...
    //初始化编辑属性
    initEditAttribute() {
      fetch(localhost + date + "/" + city1 + "/" + city2 + "/d401")
          .then((res) => res.json())
          .then((data) => {
